 ...
 *
 * $Log: m36_calib.c,v $
 * Revision 1.5  2008/01/30 10:50:45  ts
 * Fix: let define NR_CHANS = 16, not 1 !
 * last work checkin
 *
...
#define NR_RAW_VALUES	10 /* 10000 */ 	/* max. 16384 ! */
...
/************************************************************************************
 ** M36N Config Routine - setting UUT Parameters
 *  \param pathX      \IN   UUT device path
 *  \param chanX	  \IN	Channel number
 *  \param modeX	  \IN	measuring mode
 *  \param gainX	  \IN	gain factor
 *  \param trigX	  \IN	trigger mode
 *  \return	          0 on success or error code
*/
static int32 m36n_config(int32 pathX, int32 chanX, int32 modeX, int32 gainX)
{
		/* set current channel */
	if ((M_setstat(pathX, M_MK_CH_CURRENT, chanX)) < 0) {
		PrintError("setstat M_MK_CH_CURRENT");
		goto abort;
	}

	/* set measuring mode */
	if ((M_setstat(pathX, M36_BIPOLAR, modeX)) < 0) {
...
//	if (gainX != 4) {
		/* set gain */
		if ((M_setstat(pathX, M36_CH_GAIN, gainX)) < 0) {
			PrintError("setstat M36_CH_GAIN");
			goto abort;
		}
//	}
	/* set trigger mode */
	if ((M_setstat(pathX, M36_EXT_TRIG, 0)) < 0) {
		PrintError("setstat M36_EXT_TRIG");
		goto abort;
	}

	/* enable channel */
	if ((M_setstat(pathX, M36_CH_ENABLE, 1)) < 0) {
 ...
 *  \return	 -
 */
static void config_a3850_testadapter(int var, int pol, int gn, int v, int chan, int onoff)
{
	int timecal;
	...
	//			tu_print(WDW_MSG, 0, 1 + chan, G_text);
				UOS_Delay(timecal);
...
//	int on,off;
	...
	/* ausgeklammert */
	sprintf(G_text,"                ==== 1. collecting Raw Values: Channels 0 - 15  =====        ");
...
#if TEST					
#if 1
					/* Config Terminal / Calibrator + A3850 on*/				
					switch( vari )	{
							case M36N_VAR_00:	
								a3850_chan_16(G_pathM56_1,C_ON);
								sprintf(G_text, "Raw data collecting %s GAIN %02d voltage input %3.5f ",	p ? " BIPOLAR":"UNIPOLAR",
										1 << g, G_voltFSR[p][g]* G_fakt[v]);
								tu_print(WDW_MSG, 0, 1, G_text);
								/* setup time for a3850 */
           						UOS_Delay(500);
			                    setCalibDevice(G_voltFSR[p][g] * G_fakt[v]);
								break;
							...
							case M36N_VAR_02: 
								a3850_chan_step(G_pathM56_1,c,C_ON);
								sprintf(G_text, "Raw data collecting %s GAIN %02d current input %3.5f ",	p ? " BIPOLAR":"UNIPOLAR",
										1 << g, G_currFSR[p][g]* G_fakt[v]);
								tu_print(WDW_MSG, 0, 1, G_text);
           						UOS_Delay(1100);
			                    setCalibDevice(G_currFSR[p][g] * G_fakt[v]);
								break;
					
							case M36N_VAR_03: 
								a3850_chan_diff_step(G_pathM56_1,c,C_ON);
								a3850_diff(G_pathM56_2,C_OFF);
								sprintf(G_text, "Raw Data collecting %s GAIN %02d current input %3.5f ",	p ? " BIPOLAR":"UNIPOLAR",
										1 << g, G_currFSR[p][g]* G_fakt[v]);
								tu_print(WDW_MSG, 0, 1, G_text);							
...
//					UOS_Delay(100);
					...
					for (j = 0; j < NR_RAW_VALUES; j++) {
						...
						/* printf("J=%d  c=[%d],v=[%d],g=[%d],p=[%d], G_n_raw = 0x%x \n",j,c,v,g,p,G_n_raw[p][g][v][c]);	 */
					}/* j NR_RAW_VALUES */
					G_n_raw[p][g][v][c] /= NR_RAW_VALUES;
					printf("c=[%d] v=[%d],g=[%d],p=[%d], G_n_raw = 0x%x (%d)\n",c,v,g,p, G_n_raw[p][g][v][c], G_n_raw[p][g][v][c]);
					/* printf("0x%x \n", G_n_raw[p][g][v][c] ); */
#if TEST					
#if 1					/* Config Terminal / Calibrator + A3850 off*/				
					switch( vari ){
							case M36N_VAR_00:
											tu_print(WDW_MSG, 0, 3 + c, "CH[%02d] : %05x",c,G_n_raw[p][g][v][c]);
//											a3850_chan_16(G_pathM56_1,C_OFF);
											break;
							...
							case M36N_VAR_02:
											tu_print(WDW_MSG, 0, 3 + c, "CH[%02d] : %05x",c,G_n_raw[p][g][v][c]);
											setCalibDeviceOff(WAVETEK);
											a3850_chan_step(G_pathM56_1,c,C_OFF);
											break;
							...
							case M36N_VAR_03: 
											...
											a3850_chan_diff_step(G_pathM56_1,c,C_OFF);
											a3850_diff(G_pathM56_2,C_OFF);
											break;
					}/* Config Calibrator + a3850 Test adapter */
#endif
...
{
	int32 p=0, g=0, c=0, offs=0, xa=0, xb=0, ya=0, yb=0, tablegain = 0;
	...
	for (p = 0; p < NR_POLAR; p++) { /* 0:= unipol 1:= bipol */
        for (g = gain_start; g <=gain_end; g++) { /* Gain: 1,2,4,8,16 */
			for (c = 0; c < nr_channels; c++  ) {
				...
				/* point B(x/y) is either taken at -FSR/2 V(Bipol) or at 0V (unipolar) */
...
#warning remove this
					...
					printf("%s G=%02d C=%02d| xa-xb:%d ya-yb:%d -> delta %f Of = %d Gc = %05x\n", p ? "BI":"UN", 1 << g, c, xa-xb,ya-yb, offs, gn);



#endif
				}
/* 				printf("Hier habe ich folgende G_n_raw[p][g][v][c]:\n"); */
/* 				for (p = 0; p < NR_POLAR; p++) { /\* 0:= unipol 1:= bipol *\/ */
/* 					for (g = 0; g <=4; g++) { /\* Gain: 1,2,4,8,16 *\/ */
/* 						for (c = 0; c < nr_channels; c++  ) { */
...
/* 							for (pt= */
				...
				/* ts: Attention! every gain > 3 is by FPGA design Area 7! */
				/* so, care to write the x16 gain values at area offset 7!*/
				if ( ( g >= 0 ) && ( g <= 3 ) ) 
				{
					tablegain = g;
				} 
				...
				pCorr[ ((p<<8)+(tablegain<<5)+(c<<1)) + 0 ] = (int32)o_s;
				pCorr[ ((p<<8)+(tablegain<<5)+(c<<1)) + 1 ] = (int32)g_s;
...
int main(int argc, char *argv[])
{
	...
	cls();
//	sprintf(G_text,"MEN M36N Calibrate tool built %s %s\n", __DATE__, __TIME__ );
//	sprintf(G_text,"MEN M36N Calibrate tool\n");
//	tu_init_screen( G_text, 0, 0, 0 );
	...
	G_variant = get_m36_variant(G_pathM36);
	...
	m36n_calibrate(G_variant, verbose, unity );
