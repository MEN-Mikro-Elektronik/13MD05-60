...
--------------------
01j,12oct05,dlk  Address SPR #113348. Minor optimizations, other corrections.
...
$Log: m5200FecEnd.c,v $
Revision 1.25  2011/01/20 13:03:08  rlange
R: TSR871624 (reported by Customer)
M: Changings made by Mihai Culea (mc)

Revision 1.24  2010/10/14 13:17:47  UFranke
R: removed unnecessary code
M: m5200FecStop() do not call SYS_FEC_INT_DISCONNECT()

Revision 1.23  2010/04/23 10:35:29  rlange
R: Z077End driver BSP interface changed
M: Changed type of specific MII_REGS structure collided with define
   in miiLib.h

Revision 1.22  2007/09/13 08:55:34  cs
removed:
  - include of net/if_subr.h
    reason: file not existant for vxWorks 5.x and not actually needed/obsolete

Revision 1.21  2006/12/13 13:34:55  ufranke
removed
 - disable of MII -> see FEC_END_MII_MAN_DIS

Revision 1.20  2006/09/11 10:39:14  cs
commented out unused m5200FecMblkWalk()
moved declaration of external function m5200FecMiiRead() to m5200FecEnd.h

Revision 1.19  2006/07/27 08:41:18  ufranke
readded
 + ETHER_CRC_LEN for VxWorks < 6.0

Revision 1.18  2006/07/12 11:00:42  ufranke
fixed
...
merged
 - WRS SPR #113348 with MEN fixes
Revision 1.17  2006/07/12 10:39:34  ufranke
intermediate checkin

Revision 1.16  2006/02/13 15:29:20  UFRANKE
cosmetics

Revision 1.15  2006/02/03 14:44:38  UFRANKE
changed
 - TASK_ETH_BD_ to TASK_BD_ for BestCOMM 2.2

changed
 - to BestComm Version 2.2
 - BestComm FEC Rx priority is lower then Tx now
   to avoid Tx underruns
Revision 1.14  2006/01/26 10:09:17  UFRANKE
added
 + display VxWorks version compiled for

Revision 1.13  2005/12/19 20:20:22  cs
added support for VxWorks 6.1
merged some fixes from WindRiver driver

Revision 1.12  2005/10/04 17:41:04  UFranke
changed
 - moved LOCAL definitions away from this header into the driver
 - get VxWorks version information from version.h
 - changed back initialization of cache handling functions

Revision 1.11  2005/08/29 13:35:53  CSchuster
changed initialization of cache handling functions for VxWorks 6.x

Revision 1.10  2005/04/08 10:57:07  kp
Fix problem "netJobAdd: ring buffer overflow!". Use txHandlerScheduled
flag to avoid scheduling of FecTxHandle too many times

Revision 1.9  2005/04/05 15:00:49  ufranke
changed
 - mgt5200IntrCtl.h not longer BSP specific

Revision 1.8  2005/04/05 10:03:27  ufranke
cosmetics

Revision 1.7  2005/04/05 10:01:19  ufranke
fixed
 - full duplex mode flag
   get it from phyMode put into phyFlags
   if BSP specific auto negotiation routine
changed
 - m5200FecMibShow() printf always

1.6  20.09.2004 12:32:47 by ufranke
changed
 - logMsg() for debug disabled

1.5  28.06.2004 15:17:33 by ufranke
cosmetics

1.4  15.03.2004 12:17:16 by UFranke
change
 - increased FEC_END_BD_ALIGN from 0x20 to 0x80
 - increased FEC_END_BD_LOAN_NUM to 96
 - use MEN MPC5200 int ctrl
 - export MII read/write for external autonegotiation
 - if no free TbdNum wait a tick, free Tbd and try again
added
 + MEN DBG
 + display intput error counter

1.3  04.03.2004 09:56:12 by UFranke
changed
 - include m5200.h, m5200Sdma.h from BSP specific path
 - use MEN MPC5200 interrupt controller driver
 - increase FEC_END_BD_ALIGN and FEC_END_BD_LOAN_NUM
 - export MII write/read functions

1.2  02.03.2004 11:09:13 by UFranke
IceCube 7pkr

1.1  01.03.2004 09:50:51 by UFranke
...
#ifdef BESTCOMM_API
#if (BESTCOMM_API == 10)
#define BESTCOMM_API_10
#endif /* BESTCOMM_API == 10 */
#endif /* BESTCOMM_API */

#undef NO_RX_INT
#undef NO_TX_INT

UINT32  m5200FecEndDbg = FEC_END_DBG_ERR | FEC_END_DBG_RESTART | FEC_END_DBG_START;
...
#else
UINT32  m5200FecRxLgErr = 0x0;
UINT32  m5200FecRxNoErr = 0x0;
UINT32  m5200FecRxCrcErr = 0x0;
UINT32  m5200FecRxTrErr = 0x0;
#endif /* BESTCOMM_API */
...
#else /* BESTCOMM_API */
#define FEC_END_RX_LG_ADD                       \
    do {m5200FecRxLgErr++;}while (0)

#define FEC_END_RX_NO_ADD                       \
    do {m5200FecRxNoErr++;}while (0)

#define FEC_END_RX_CRC_ADD                      \
    do {m5200FecRxCrcErr++;}while (0)

#define FEC_END_RX_TR_ADD                       \
    do {m5200FecRxTrErr++;}while (0)
#endif /* BESTCOMM_API */
...
u_int32         fecDbgLevel = 0xc0008000;
static DBG_HANDLE   *fecDbgHdl = NULL;        /* debug handle */
#define DBG_MYLEVEL     fecDbgLevel
#define DBH             fecDbgHdl
...
#define FEC_END_NEXT_RBD(pDrvCtrl, pRbd)                   \
    (pRbd) = (FEC_END_RBD_ID) ((pDrvCtrl)->rbdBase +       \
                   (((pDrvCtrl)->rbdIndex) * FEC_END_RBD_SZ))
...
#ifndef BESTCOMM_API
#define FEC_END_RX_ACTIVATE                     \
    SDMA_TASK_ENABLE (FEC_RECV_TASK_NO);

#define FEC_END_TX_ACTIVATE                     \
...
#ifndef BESTCOMM_API
/* these variables must be together in SDRAM. They are used by the
   SDMA tasks */
LOCAL unsigned long ETH_PARAM_BASE;
LOCAL unsigned long SDMA_TBD_BASE;
LOCAL unsigned long SDMA_TBD_NEXT;
LOCAL unsigned long SDMA_RBD_BASE;
LOCAL unsigned long SDMA_RBD_NEXT;
LOCAL unsigned long SDMA_RTBD_BASE;
#endif
...
LOCAL void      m5200FecHandleRecvInt (DRV_CTRL *pDrvCtrl, int * pRxCountdown);
#if 0
...
LOCAL STATUS    m5200FecMiiDiag (DRV_CTRL *pDrvCtrl, UINT8 phyAddr);
...
END_OBJ* m5200FecEndLoad
	...
	printf( "m5200FecEnd.c compiled for VxWorks %s\n", RUNTIME_VERSION );
    ...
    pDrvCtrl->phyInfo = (M5200FEC_PHY_INFO *) calloc (sizeof (M5200FEC_PHY_INFO), 1);
   ...
   /* memory initialization */
...
#ifndef BESTCOMM_API
    ETH_PARAM_BASE = (UINT32)FEC_END_VIRT_TO_PHYS(pDrvCtrl->pBufBase);
    /* long word aligned */
    ETH_PARAM_BASE = ETH_PARAM_BASE + ETH_PARAM_BASE % 8;
    SDMA_TBD_BASE  = ETH_PARAM_BASE;
    SDMA_TBD_NEXT  = ETH_PARAM_BASE + 4;
    SDMA_RBD_BASE  = ETH_PARAM_BASE + 8;
    SDMA_RBD_NEXT  = ETH_PARAM_BASE + 12;
    SDMA_RTBD_BASE = ETH_PARAM_BASE + 16;
#endif
...
#else
    pDrvCtrl->ivecRDMA    = SDMA_TASKNO_TO_IVEC(FEC_RECV_TASK_NO);
    pDrvCtrl->ivecWDMA    = SDMA_TASKNO_TO_IVEC(FEC_XMIT_TASK_NO);
#endif
    ...
    DBGWRT_1( ( DBH, "m5200FecEndLoad:...Done\n" ));
    return (&pDrvCtrl->endObj);

errorExit:
	taskDelay( sysClkRateGet () );
    DBGWRT_ERR((DBH,"*** m5200FecEndLoad: failed\n" ));
...
LOCAL STATUS m5200FecUnload
...
#ifndef BESTCOMM_API
    if ((FEC_END_FLAG_ISSET (FEC_END_OWN_MEM)) && (pDrvCtrl->pBufBase != NULL))
	...
	/*mc*/
	if ((pDrvCtrl->pClBlkArea) != NULL)
        free (pDrvCtrl->pClBlkArea);
	/*mc*/
    ...
    switch ((int) pDrvCtrl->pBufBase)
        ...
        case NONE :              /* we must obtain it */
            ...
            if (!CACHE_DMA_IS_WRITE_COHERENT ())
                {
                FEC_END_LOG (FEC_END_DBG_LOAD, ("m5200FecInitMem: shared \n\
            ...
            if (pDrvCtrl->pBufBase == NULL)      /* no memory available */
                {
                FEC_END_LOG (FEC_END_DBG_LOAD, ("m5200FecInitMem: could not \n\
                                    obtain memory\n"),
                                    1, 2, 3, 4, 5, 6);
                return (ERROR);
                }
            ...
            FEC_END_FLAG_SET (FEC_END_OWN_MEM);
        ...
        default :                  /* the user provided an area */
            ...
            if (pDrvCtrl->bufSize == 0)
                {
                FEC_END_LOG (FEC_END_DBG_LOAD, ("m5200FecInitMem: not enough \n\
                                    memory\n"),
                                    1, 2, 3, 4, 5, 6);
                return (ERROR);
                }
            ...
            */
            ...
            if (FEC_END_FLAG_ISSET (FEC_END_INV_TBD_NUM) &&
                FEC_END_FLAG_ISSET (FEC_END_INV_RBD_NUM))
                {
                pDrvCtrl->tbdNum = (FEC_END_MEM_SZ (pDrvCtrl) /
                            (FEC_END_TBD_SZ + FEC_END_RBD_SZ));

                pDrvCtrl->rbdNum = pDrvCtrl->tbdNum;
                }
            else if (FEC_END_FLAG_ISSET (FEC_END_INV_TBD_NUM))
                {
                pDrvCtrl->tbdNum = ((FEC_END_MEM_SZ (pDrvCtrl)
                            - FEC_END_RBD_MEM (pDrvCtrl))
                            / FEC_END_TBD_SZ);
                }
            else if (FEC_END_FLAG_ISSET (FEC_END_INV_RBD_NUM))
                {
                pDrvCtrl->rbdNum = ((FEC_END_MEM_SZ (pDrvCtrl)
                            - FEC_END_TBD_MEM (pDrvCtrl))
                            / FEC_END_RBD_SZ);
                }
            else
                {
                if (FEC_END_MEM_SZ (pDrvCtrl) < bdSize)
                    {
                    FEC_END_LOG (FEC_END_DBG_LOAD, ("m5200FecInitMem: not enough \n\
                                memory\n"),
                                1, 2, 3, 4, 5, 6);
            ...
            if ((pDrvCtrl->tbdNum <= FEC_END_TBD_MIN) ||
                    (pDrvCtrl->rbdNum <= FEC_END_RBD_MIN))
                {
                FEC_END_LOG (FEC_END_DBG_LOAD, ("m5200FecInitMem: not enough \n\
                                memory\n"),
                                1, 2, 3, 4, 5, 6);
            ...
            FEC_END_FLAG_CLEAR (FEC_END_OWN_MEM);
...
void m5200FecRxPoll
    ...
    int rxCountdown;
        ...
        {
        taskDelay (1);
	rxCountdown = 10000; /* ridiculously big */
	m5200FecHandleRecvInt(pDrvCtrl, &rxCountdown);
        }
    }
#endif
...
LOCAL STATUS m5200FecStart
    ...
    if( _func_m5200FecPhyInit != m5200FecPhyInit )
    {
		/* do the same as m5200FecMiiAnCheck() */
	    /* set 100MBS from phyMode into phyFlags */
	    if (pDrvCtrl->phyInfo->phySpeed == FEC_END_100MBS)
			FEC_END_PHY_FLAGS_SET (FEC_END_PHY_100);
    	else
			FEC_END_PHY_FLAGS_CLEAR (FEC_END_PHY_100);

	    /* set FD from phyMode into phyFlags */
...
LOCAL STATUS m5200FecStop
    ...
    if (END_FLAGS_GET(&pDrvCtrl->endObj) & (IFF_UP | IFF_RUNNING))
	    ...
	    DBGWRT_ERR((DBH,"%s: SYS_FEC_INT_DISCONNECT disabled\n", __FUNCTION__ ));
...
LOCAL void m5200FecInt
    ...
    FEC_END_LOG (FEC_END_DBG_INT, ("m5200FecInt: event 0x%x, status 0x%x\n"),
                   ...
                   0, 0, 0, 0);
	DBGWRT_1((DBH,"m5200FecInt: event 0x%x, status 0x%x\n",
				   (event & FEC_END_EVENT_MSK), status,
				   0, 0, 0, 0
	        ));
    ...
    if ((event & FEC_END_EVENT_BERR) == FEC_END_EVENT_BERR)
        ...
        FEC_END_LOG (FEC_END_DBG_ERR, ("m5200FecInt: bus error, restart chip\n"),
                           0, 0, 0, 0, 0, 0);
		DBGWRT_ERR((DBH,"m5200FecInt: bus error, restart chip\n"));
    ...
    if ((event & FEC_END_EVENT_FIFO_ERR) == FEC_END_EVENT_FIFO_ERR)
        ...
        FEC_END_LOG (FEC_END_DBG_ERR,
                     ...
                     status2, 0, 0, 0, 0, 0);
		DBGWRT_ERR((DBH,"*** m5200FecInt: TxFIFO error (status: 0x%x), clearing\n", status2 ));
    ...
    if ((event & FEC_END_EVENT_RFIFO_ERR) == FEC_END_EVENT_RFIFO_ERR)
        ...
        FEC_END_LOG (FEC_END_DBG_ERR,
                     ...
                     status2, 0, 0, 0, 0, 0);
		DBGWRT_ERR((DBH,"*** m5200FecInt: RxFIFO error (status 0x%x), clearing\n", status2 ));
#ifdef  BESTCOMM_API
#ifdef NO_RX_INT
        if (pDrvCtrl->rxPollTID != ERROR)
            {
    ...
    if ((event & FEC_END_EVENT_BABT) == FEC_END_EVENT_BABT)
        ...
        FEC_END_LOG (FEC_END_DBG_ERR, ("m5200FecInt: babbling tx error\n"),
                           0, 0, 0, 0, 0, 0);
		DBGWRT_ERR((DBH,"*** m5200FecInt: babbling tx"));
        }
    ...
    if ((event & FEC_END_EVENT_BABR) == FEC_END_EVENT_BABR)
        ...
        FEC_END_LOG (FEC_END_DBG_ERR, ("m5200FecInt: babbling rx error\n"),
                           0, 0, 0, 0, 0, 0);
		DBGWRT_ERR((DBH,"*** m5200FecInt: babbling rx"));
        }
    ...
    if ((event & FEC_END_EVENT_LCOL) == FEC_END_EVENT_LCOL)
        ...
        FEC_END_LOG (FEC_END_DBG_ERR, ("m5200FecInt: late collision error\n"),
                           0, 0, 0, 0, 0, 0);
		DBGWRT_ERR((DBH,"*** m5200FecInt: late collision"));
        }
    ...
    if ((event & FEC_END_EVENT_COL_RL) == FEC_END_EVENT_COL_RL)
        ...
        FEC_END_LOG (FEC_END_DBG_ERR, ("m5200FecInt: collision retry error\n"),
                           0, 0, 0, 0, 0, 0);
		DBGWRT_ERR((DBH,"*** m5200FecInt: collision retry"));
        }
    ...
    if ((event & FEC_END_EVENT_XFIFO_UN) == FEC_END_EVENT_XFIFO_UN)
        ...
        FEC_END_LOG (FEC_END_DBG_ERR, ("m5200FecInt: TxFIFO underrun error\n"),
                           0, 0, 0, 0, 0, 0);
		DBGWRT_ERR((DBH,"*** m5200FecInt: TxFIFO underrun"));
        }
    ...
    if ((event & FEC_END_EVENT_HB) == FEC_END_EVENT_HB)
        ...
        FEC_END_LOG (FEC_END_DBG_ERR, ("m5200FecInt: heartbeat check fail\n"),
                           0, 0, 0, 0, 0, 0);
		DBGWRT_ERR((DBH,"*** m5200FecInt: heartbeat check"));
        }
...
LOCAL void m5200FecRdmaInt
    ...
    if (netJobAdd ((FUNCPTR) m5200FecRxHandle, (int) pDrvCtrl, 0, 0, 0, 0)
	!= OK)
	{
#ifdef BESTCOMM_API
...
LOCAL void m5200FecWdmaInt
...
#else
    SDMA_INT_DISABLE (FEC_XMIT_TASK_NO);
    SDMA_CLEAR_IEVENT (FEC_XMIT_TASK_NO);
#endif
	...
	DBGWRT_3((DBH,"WI %d mask=%x\n", pDrvCtrl->xmitTaskNo,
		  *(u_int32*)SDMA_INT_MASK));

	/*
	 * kp: avoid DOS attack to netjob, don't schedule FecTxHandle
	 * if already scheduled
	 */
	if( !pDrvCtrl->txHandlerScheduled ){

    if (netJobAdd ((FUNCPTR) m5200FecTxHandle, (int) pDrvCtrl, 0, 0, 0, 0)
	!= OK)
	{
		DBGWRT_1((DBH,"***netJobAdd W\n"));

#ifdef BESTCOMM_API
	SDMA_INT_ENABLE (SDMA_INT_MASK, pDrvCtrl->xmitTaskNo );
#else
	SDMA_INT_ENABLE (FEC_XMIT_TASK_NO);
#endif
	}
	    else
		pDrvCtrl->txHandlerScheduled = TRUE;
	}
    }
...
LOCAL STATUS m5200FecSend
    ...
    /*
     * Set the packet type to multicast or unicast. Assume the first
     * M_BLK has data of non-zero length.
     */
    ...
    if ((pDrvCtrl->cleanTbdNum) == 0)
        ...
        if ((retVal = m5200FecPktCopyTransmit (pDrvCtrl, pMblk,
					       fragNum, pktType)) != OK)
            {
            goto m5200FecSendError;
            }
...
/* this version of bestcomm fec code requires longword alignment */
	&& ((UINT32)(pMblk->mBlkHdr.mData)%4 == 0) && !m5200FecForceCopy
#endif
       )
        {
        /* transmit the packet in zero-copy mode */

        if ((retVal = m5200FecPktTransmit (pDrvCtrl, pMblk, fragNum,
					   pktType)) != OK)
            {
            goto m5200FecSendError;
            }
        ...
        if ((retVal = m5200FecPktCopyTransmit (pDrvCtrl, pMblk, fragNum,
					       pktType)) != OK)
            {
            goto m5200FecSendError;
            }
...
/***************************************************************************
*
* TaskStart2 - Start an initialized task running, leave interrupt enable alone
*
* This routine is similar to the BestComm 'capi' routine TaskStart(), except
* that it leaves the interrupt enable state of the task unmodified. It
* is used to start a new transmit without either enabling or disabling the
* transmit complete interrupt.  (We enable transmit complete interrupts
* in m5200FecStart(), disable them when we post a transmit cleanup job
* from m5200FecWdmaInt(), and only re-enabled them again when the transmit
* cleanup job completes.
*
* \is
* \i taskId
*    Task handle passed back from a successful TaskSetup()
* \i autoStartEnable
*    Boolean indicating whether autostart bit is enabled. If this is set
*    then the parameter autoStartTask defines the task to auto start.
* \i autoStartTask
*    TaskId for task to autostart. If autoStartEnable is not set then this
*    parameter is a don't care.
* \ie
*
...
LOCAL STATUS m5200FecPktTransmit
    ...
    if (
#ifdef NO_TX_INT
	TaskStart (pDrvCtrl->xmitTaskNo, FALSE,
		   pDrvCtrl->xmitTaskNo, FALSE) != TASK_ERR_NO_ERR
#else
	TaskStart2( pDrvCtrl->xmitTaskNo, TASK_AUTOSTART_ENABLE,
		    pDrvCtrl->xmitTaskNo) != TASK_ERR_NO_ERR
#endif
       )
        {
        FEC_END_LOG (FEC_END_DBG_TX,
		     ("m5200FecPktTransmit: failed to start TX task\n"),
                            1, 2, 3, 4, 5, 6);
        return (ERROR);
        }
#else
...
LOCAL STATUS m5200FecPktCopyTransmit
	...
	TaskStart2( pDrvCtrl->xmitTaskNo, TASK_AUTOSTART_ENABLE,
        ...
        {
        FEC_END_LOG (FEC_END_DBG_TX,
		     ("m5200FecPktCopyTransmit: failed to start TX task\n"),
                            1, 2, 3, 4, 5, 6);
        retVal = ERROR;
        goto errorExit;
        }
    ...
    while (pDrvCtrl->cleanTbdNum < pDrvCtrl->tbdNum)
        ...
        while (pTempList != NULL)
            ...
            if (pDrvCtrl->usedTbdIndex == (pDrvCtrl->tbdNum - 1))
                {
                FEC_END_BD_WORD_WR (pUsedTbd, FEC_END_BD_STAT_OFF,
                            FEC_END_TBD_WRAP);
                }
            else
#endif
...
LOCAL UINT32 m5200FecTbdCheck
    ...
    if ((tbdStatus & FEC_END_TBD_LC) == FEC_END_TBD_LC)
        {
        FEC_END_LOG (FEC_END_DBG_TX, ("m5200FecTbdCheck: late collision\n"),
                        1, 2, 3, 4, 5, 6);
        FEC_END_TX_LC_ADD;
        }

    if ((tbdStatus & FEC_END_TBD_RL) == FEC_END_TBD_RL)
        {
        FEC_END_LOG (FEC_END_DBG_TX, ("m5200FecTbdCheck: retransmit limit"),
                        1, 2, 3, 4, 5, 6);
        FEC_END_TX_RL_ADD;
        }

    if ((tbdStatus & FEC_END_TBD_UN) == FEC_END_TBD_UN)
        {
        FEC_END_LOG (FEC_END_DBG_TX, ("m5200FecTbdCheck: tx underrun\n"),
                        1, 2, 3, 4, 5, 6);
        FEC_END_TX_UR_ADD;
        }

    if ((tbdStatus & FEC_END_TBD_CSL) == FEC_END_TBD_CSL)
        {
        FEC_END_LOG (FEC_END_DBG_TX, ("m5200FecTbdCheck: carrier sense \n"),
                        1, 2, 3, 4, 5, 6);
        FEC_END_TX_CSL_ADD;
        }
#endif /* FEC_END_DBG */
    ...
    if (tbdStatus & (FEC_END_TBD_LC |
                     FEC_END_TBD_RL |
                     FEC_END_TBD_UN |
                     FEC_END_TBD_CSL))
        {
        return (FEC_END_TBD_ERROR);
        }
#endif
...
#ifndef BESTCOMM_API
    pDrvCtrl->tbdBase = pTbd = (FEC_END_TBD_ID) pDrvCtrl->pBufBase + 16;
#else
...
LOCAL void m5200FecRxHandle
    ...
    int		rxCountdown = 16;  /* at most this many packets per net job */
    UINT32	pending;
    ...
    intLevel = intLock(); /* is this needed? */
     ...
     *
     * Also, if rxCountdown has decreased to zero, it's possible that
     * there's a descriptor ready which we haven't examined, so we
     * reschedule ourselves via another netJobAdd() to handle that
     * and avoid hogging tNetTask.
     * (Even if a descriptor's not ready, since we've serviced a good number
     * of packets in this job, it's a fairly good bet that if we
     * reschedule ourselves, another packet will be ready by the time we
     * get to run again.)
     *
     * We know that RX SDMA interrupts are disabled (masked) here because
     * we disabled them in m5200FecRdmaInt(). TaskIntStatus() uses
     * 'pending = SDMA_INT_PENDING(SDMA_INT_PEND, SDMA_INT_MASK);'
     * so it would never see the pending but masked interrupt, which is
     * what we want to know about. So we shouldn't use TaskIntStatus().
     */

#ifdef BESTCOMM_API
    pending = *(volatile UINT32 *)SDMA_INT_PEND;
    if (rxCountdown == 0 || SDMA_INT_TEST (pending, pDrvCtrl->recvTaskNo))
        {
        TaskIntClear( pDrvCtrl->recvTaskNo );
        intUnlock (intLevel);
	if (rxCountdown > 0)
	...
	/* if the netJobAdd() fails, fall through to re-enable interrupts */
	intLevel = intLock();  /* ugly */
        }
#else
    if (rxCountdown == 0 ||
	SDMA_GET_PENDINGBIT(FEC_RECV_TASK_NO) != 0)
        {
        SDMA_CLEAR_IEVENT (FEC_RECV_TASK_NO);
        intUnlock (intLevel);
	if (rxCountdown > 0)
...
LOCAL void m5200FecTxHandle
    ...
    if (TaskIntStatus( pDrvCtrl->xmitTaskNo ) == pDrvCtrl->xmitTaskNo)
        ...
        TaskIntClear( pDrvCtrl->xmitTaskNo );
#else
    if (SDMA_GET_PENDINGBIT(FEC_XMIT_TASK_NO) != 0)
        {
        SDMA_CLEAR_IEVENT (FEC_XMIT_TASK_NO);
#endif
        intUnlock (intLevel);
        ...
        SDMA_INT_ENABLE (FEC_XMIT_TASK_NO);
#endif
		pDrvCtrl->txHandlerScheduled = FALSE;
...
*
* This routine is run in netTask's context. It is called by
* m5200FecRxHandle() so that it could handle receive packets at task level.
*
* RETURNS: TRUE if descriptors are known to be ready, FALSE otherwise.
*/
...
#ifdef BESTCOMM_API_10
    UINT16      rbdLength = 0;  /* holder for the length of this rbd */
#else
    UINT16      rbdStatus = 0;  /* holder for the status of this rbd */
#endif
    int		rxCountdown = *pRxCountdown;
...
#ifdef BESTCOMM_API_10
        /* read the RBD length word */

        FEC_END_BD_WORD_RD (pUsedRbd, FEC_END_BD_LEN_OFF, rbdLength);
...
#else
    if ((rbdStatus & FEC_END_RBD_LG) == FEC_END_RBD_LG)
        {
        FEC_END_LOG (FEC_END_DBG_RX, ("m5200FecReceive: len violation\n"),
                      1, 2, 3, 4, 5, 6);
        FEC_END_RX_LG_ADD;
        }

    if ((rbdStatus & FEC_END_RBD_NO) == FEC_END_RBD_NO)
        {
        FEC_END_LOG (FEC_END_DBG_RX, ("m5200FecReceive: not aligned\n"),
                      1, 2, 3, 4, 5, 6);
        FEC_END_RX_NO_ADD;
        }

    if ((rbdStatus & FEC_END_RBD_CRC) == FEC_END_RBD_CRC)
        {
        FEC_END_LOG (FEC_END_DBG_RX, ("m5200FecReceive: CRC error\n"),
                      1, 2, 3, 4, 5, 6);
        FEC_END_RX_CRC_ADD;
        }

    if ((rbdStatus & FEC_END_RBD_TR) == FEC_END_RBD_TR)
        {
        FEC_END_LOG (FEC_END_DBG_RX, ("m5200FecReceive: trunc frame \n"),
                      1, 2, 3, 4, 5, 6);
        FEC_END_RX_TR_ADD;
        }
#endif
...
#ifndef BESTCOMM_API
        FEC_END_BD_LONG_WR (pUsedRbd, FEC_END_BD_ADDR_OFF, (UINT32) pData);
        }
#else
        }
    else
...
#ifndef BESTCOMM_API
    FEC_END_BD_WORD_WR (pUsedRbd, FEC_END_BD_LEN_OFF, 0);

    /* up-date the status word: treat the last RBD in the ring differently */

    if ((pDrvCtrl->rbdIndex) == (pDrvCtrl->rbdNum - 1))
        {
        FEC_END_BD_WORD_WR (pUsedRbd, FEC_END_BD_STAT_OFF,
                            (FEC_END_RBD_WRAP |
                             FEC_END_RBD_INT | FEC_END_RBD_EMPTY));
        }
    else
        {
        FEC_END_BD_WORD_WR (pUsedRbd, FEC_END_BD_STAT_OFF,
...
#ifdef BESTCOMM_API_10
    TaskBDAssign( pDrvCtrl->recvTaskNo, pData, NULL, 0, 0 );
#else
...
LOCAL STATUS m5200FecRbdInit
    ...
    pDrvCtrl->rbdBase = pRbd = (FEC_END_RBD_ID) (pDrvCtrl->pBufBase
                         + FEC_END_TBD_MEM (pDrvCtrl) + 16);
#endif
    ...
    for (ix = 0; ix < pDrvCtrl->rbdNum; ix++)
        ...
        */
        ...
        FEC_END_BD_WORD_WR (pRbd, FEC_END_BD_STAT_OFF,
                    FEC_END_RBD_INT | FEC_END_RBD_EMPTY);

        FEC_END_BD_WORD_WR (pRbd, FEC_END_BD_LEN_OFF, 0);
#endif
...
#ifndef BESTCOMM_API
        FEC_END_BD_LONG_WR (pRbd, FEC_END_BD_ADDR_OFF, (UINT32) pData);
    ...
    FEC_END_BD_WORD_WR (pRbd, FEC_END_BD_STAT_OFF,
            (FEC_END_RBD_WRAP | FEC_END_RBD_INT | FEC_END_RBD_EMPTY));
#endif
...
LOCAL STATUS m5200FecBdFree
	...
	/* free the TX buffers descriptors - *mc*/

	for (ix = 0; ix < pDrvCtrl->tbdNum; ix++)
	{
		if(pDrvCtrl->pTbdList [ix] != NULL)
		{
		free(pDrvCtrl->pTbdList [ix]);
		}
	}
...
LOCAL STATUS m5200FecPrePhyConfig
   ...
   /*
    ...
    csrVal |= (FEC_END_EVENT_GRA | FEC_END_EVENT_MII | FEC_END_EVENT_BABR |
...
#else
    FEC_END_CSR_WR (FEC_END_x_wmrk_OFF, FEC_XFIFO_WMRK_64);
#endif

#ifndef BESTCOMM_API
    /* clear the ethernet FIFO read/write pointer and initialize the FIFO alarm */
    FEC_END_CSR_WR (FEC_END_rfifo_alarm_OFF, 0x404);
    FEC_END_CSR_WR (FEC_END_tfifo_alarm_OFF, 0x204);
    FEC_END_CSR_WR (FEC_END_r_bound_OFF, 0x400);
    FEC_END_CSR_WR (FEC_END_r_fstart_OFF, 0x200);
    FEC_END_CSR_WR (FEC_END_x_lag_OFF, 0x0);
    FEC_END_CSR_WR (FEC_END_x_write_OFF, 0x0);
    FEC_END_CSR_WR (FEC_END_x_read_OFF, 0x0);
    FEC_END_CSR_WR (FEC_END_r_write_OFF, 0x0);
    FEC_END_CSR_WR (FEC_END_r_read_OFF, 0x0);

    FEC_END_CSR_WR (FEC_END_rfifo_rdptr_OFF, 0x0);
    FEC_END_CSR_WR (FEC_END_rfifo_wrptr_OFF, 0x0);
    FEC_END_CSR_WR (FEC_END_tfifo_rdptr_OFF, 0x0);
    FEC_END_CSR_WR (FEC_END_tfifo_wrptr_OFF, 0x0);
    FEC_END_CSR_WR (FEC_END_rfifo_lrf_ptr_OFF, 0);
    FEC_END_CSR_WR (FEC_END_rfifo_lwf_ptr_OFF, 0);
    FEC_END_CSR_WR (FEC_END_tfifo_lrf_ptr_OFF, 0);
    FEC_END_CSR_WR (FEC_END_tfifo_lwf_ptr_OFF, 0);
...
LOCAL STATUS m5200FecPostPhyConfig
    ...
    /* FEC_END_CSR_WR (FEC_END_MII_SPEED_OFF, FEC_END_MII_MAN_DIS); */
...
LOCAL STATUS m5200FecPhyInit
    ...
    for (ix = 0; ix < MII_MAX_PHY_NUM; ix++)
        ...
        if (m5200FecMiiDiag (pDrvCtrl, phyAddr) != OK)
            return (ERROR);
...
/*******************************************************************************
*
* m5200FecMiiDiag - run some diagnostics
*
...
LOCAL STATUS m5200FecMiiDiag
    ...
    if (ix >= FEC_END_PHY_MAX_WAIT)
        {
        FEC_END_LOG (FEC_END_DBG_MII, ("m5200FecMiiDiag reset fail\n"),
                        0, 0, 0, 0, 0, 0);
        return (ERROR);
        }
    ...
    if (m5200FecMiiIsolate (pDrvCtrl, phyAddr) == ERROR)
        return (ERROR);
    ...
    if (ix >= FEC_END_PHY_MAX_WAIT)
        {
        FEC_END_LOG (FEC_END_DBG_MII, ("m5200FecMiiDiag de-isolate fail\n"),
                        0, 0, 0, 0, 0, 0);
        return (ERROR);
        }

    FEC_END_LOG (FEC_END_DBG_MII, ("m5200FecMiiDiag... ends\n"),
                   0, 0, 0, 0, 0, 0);
    return (OK);
    }
...
LOCAL STATUS m5200FecPollReceive
    ...
    if (rbdStatus & FEC_END_RBD_ERR)
        {
        FEC_END_LOG (FEC_END_DBG_POLL, ("m5200FecReceive: bad rbd\n"),
                            1, 2, 3, 4, 5, 6);
...
#ifndef BESTCOMM_API
   FEC_END_RBD_ID bd = (FEC_END_RBD_ID)(pDrvCtrlDbg->rbdBase + (rbdNum * FEC_END_RBD_SZ));
#else
...
void m5200FecErrorShow
    ...
    FEC_END_LOG (FEC_END_DBG_ANY, ("RxFEmpty=0x%x RxAlarm=0x%x RxFull=0x%x \n\
                   ...
                   0);
#else
    FEC_END_LOG (FEC_END_DBG_ANY, ("RxLg=0x%x RxNo=0x%x RxCrc=0x%x \n\
                   RxOv=0x%x RxTr=0x%x\n"),
                   m5200FecRxLgErr,
                   m5200FecRxNoErr,
                   m5200FecRxCrcErr,
...
void m5200FecDrvShow
    ...
    FEC_END_LOG (FEC_END_DBG_ANY, ("rbdNum=%d recvTaskNo=%d rbdIndex=%d \n"),
                   ...
                   0, 0, 0);
#else
    FEC_END_LOG (FEC_END_DBG_ANY, ("rbdNum=%d rbdBase=0x%x rbdIndex=%d \n"),
                   pDrvCtrl->rbdNum,
...
LOCAL STATUS m5200FecSdmaTaskInit
    ...
    */
    ...
    /* set initiator priority is needed */
    if( sysSDMA->PtdCntrl & 0x00008000 /* Task Priority Mode */ )
    {
   		/* set task priority */
	    sysSDMA->IPR[pDrvCtrl->recvTaskNo] = 5;
    	sysSDMA->IPR[pDrvCtrl->xmitTaskNo] = 6;
    }
    else
    {
	   /* set initiator priority */
    	sysSDMA->IPR[3] = 5; /* FEC RX FIFO */
		sysSDMA->IPR[4] = 6; /* FEC TX FIFO */
	}

#else /* BESTCOMM_API */
    unsigned long *buf1;
    unsigned long *sram, var_table1, var_table2;

    FEC_END_LOG (FEC_END_DBG_LOAD, ("m5200FecSdmaTaskInit\n"),
                    0, 0, 0, 0, 0, 0);

    /* set initiator priority is needed */
    sysSDMA->IPR[FEC_RECV_TASK_NO] = 6; /* rx */
    sysSDMA->IPR[FEC_XMIT_TASK_NO] = 5; /* tx */

    buf1 = (unsigned long *)ETH_PARAM_BASE;
    ...
    *buf1 = SDMA_RTBD_BASE;
    ...
    /* initialize RBD base */
    *buf1 = SDMA_RTBD_BASE + pDrvCtrl->tbdNum * FEC_END_TBD_SZ;
    buf1 ++;
    ...
    *buf1 = SDMA_RTBD_BASE + pDrvCtrl->tbdNum * FEC_END_TBD_SZ;
    ...
    sram = (unsigned long *)SRAM_BASE_ADRS;

    /* get the physical address of task variable table */
    var_table1 = sram[FEC_RECV_TASK_NO * 8 + 2];
    var_table2 = sram[FEC_XMIT_TASK_NO * 8 + 2];

    /* the task parameter is saved in the first location of variable table which shall be defined according the "sas" */
    buf1  = (unsigned long *)var_table1;
    *buf1 = ETH_PARAM_BASE;

    buf1  = (unsigned long *)var_table2;
    *buf1 = ETH_PARAM_BASE;
