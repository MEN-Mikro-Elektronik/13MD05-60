...
static int m8260SioPollInput(
	SIO_CHAN * pSioChan,
	char *
);
static int m8260SioStartup(
	M8260_SCC_CHAN * pSccChan
);
...
void m8260SioResetChannel(
	...
	VUINT16 *pSCCM =
		(VUINT16 *) (immrVal + M8260_SCC_BASE + M8260_SCCM_OFFSET +
					 (scc * M8260_SCC_OFFSET_NEXT_SCC));
	...
	VUINT16 *pCHARACTER1 = (VUINT16 *) (immrVal + M8260_SCC_PRAM_BASE +
										0x50 +
										(scc *
										 M8260_SCC_PRAM_OFFSET_NEXT_PRAM));
	...
	*pBRGC |= M8260_BRGC_RST;
	...
	M8260_SCC_16_WR((pSccChan->pBdBase +
					 M8260_SCC_RCV_BD_OFF + M8260_SCC_BD_STAT_OFF), 0xB000);
	CACHE_PIPE_FLUSH();

	/* buffer length */
	M8260_SCC_16_WR((pSccChan->pBdBase +
					 M8260_SCC_RCV_BD_OFF + M8260_SCC_BD_LEN_OFF), 0x0001);
	CACHE_PIPE_FLUSH();
	...
	*pSccChan->pRBASE = (UINT16) ((UINT32) pSccChan->pBdBase & 0xffff);
	...
	M8260_SCC_16_WR((pSccChan->pBdBase +
					 M8260_SCC_TX_BD_OFF + M8260_SCC_BD_STAT_OFF), 0x3800);

	/* buffer length */

	M8260_SCC_16_WR((pSccChan->pBdBase +
					 M8260_SCC_TX_BD_OFF + M8260_SCC_BD_LEN_OFF), 0x0001);
	...
	*pSccChan->pTBASE =
		((UINT16) ((UINT32) pSccChan->pBdBase & 0xffff) + 0x08);
	...
	*pGSMR_L = (M8260_SCC_GSMRL_RDCR_X16 |
				M8260_SCC_GSMRL_TDCR_X16 | M8260_SCC_GSMRL_UART)
	...
	case SIO_BAUD_SET:
		if (arg >= 300 && arg <= 115200) {
			...
			if (--baudRate > 0xfff)
				...
				baud = (M8260_BRGC_CD_MASK & (baudRate << 1)) | M8260_BRGC_EN;
...
void m8260SioInt(
	...
	VUINT16 *pSCCE =
		(VUINT16 *) (immrVal + M8260_SCC_BASE + M8260_SCCE_OFFSET +
					 (scc * M8260_SCC_OFFSET_NEXT_SCC));
	...
	if ((*pSCCE & M8260_SCC_UART_SCCX_TX) && (pSccChan->channelMode != SIO_MODE_POLL)) {	/* ...and we're not polling */
...
LOCAL int m8260SioStartup(
	...
	M8260_SCC_16_RD((pSccChan->pBdBase +
					 M8260_SCC_TX_BD_OFF + M8260_SCC_BD_STAT_OFF), bdStatus);
...
LOCAL int m8260SioPollInput(
	...
	M8260_SCC_16_RD((pSccChan->pBdBase +
					 M8260_SCC_RCV_BD_OFF + M8260_SCC_BD_STAT_OFF), bdStatus);
