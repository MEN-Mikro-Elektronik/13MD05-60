...
--------------------
01g,19oct04,dtr  SPR 102606.
01f,06may04,mil  Removed compiler warnings.
...
IMPORT STATUS excIntConnect(
	VOIDFUNCPTR *,
	VOIDFUNCPTR
);
IMPORT STATUS excIntCrtConnect(
	VOIDFUNCPTR *,
	VOIDFUNCPTR
);

#if FALSE
#if defined(INCLUDE_WINDVIEW) || defined(INCLUDE_INSTRUMENTATION)
IMPORT int evtTimeStamp;		/* Windview 1.0 only */
#endif							/* WINDVIEW||INSTRUMENTATION */
#endif							/* FALSE */
...
LOCAL UINT32 irqCount[INTERRUPT_TABLESIZE]={0};
...
LOCAL void sysEpicIntHandler(
	void
);

#ifdef INCLUDE_EPIC_CRT_INTR
LOCAL void sysEpicCrtIntHandler(
	void
);
...
)
{
	...
	for (irq = 0; irq < EPIC_MAX_GT_IRQS; irq++) {
		...
		epicIntSourceSet(EPIC_GT_VEC_REG(irq), 0x0, 0x0, 0x0, 0x0);
	}
	...
	for (irq = 0; irq < EPIC_MAX_MSG_IRQS; irq++) {
		...
		epicIntSourceSet(EPIC_MSG_VEC_REG(irq), 0x0, 0x0, 0x0, 0x0);
	}
	...
	for (irq = 0; irq < EPIC_MAX_IPI_IRQS; irq++) {
		...
		epicIntSourceSet(EPIC_IPI_VEC_REG(irq), 0x0, 0x0, 0x0, 0x0);
	}
...
)
{
	...
	_func_intDisconnectRtn = sysEpicIntDisconnect;
...
)
{
	...
	intVal = intCpuLock();			/* lock interrupts to prevent races */

	if (sysIntTbl[(int) vector] == NULL) {
		sysIntTbl[(int) vector] = pNewHandler;	/* single int. handler case */
	}
	else {
		pCurrHandler = sysIntTbl[(int) vector];	/* multiple IRQ handler case */
...
/***********************************************************************
*
* sysEpicIntDisconnect - disconnect an interrupt handler from vector table
*
* This function disconnects an interrupt handler to the system vector table.
*
* RETURNS: OK/ERROR.
*/
...
* NOMANUAL
*
* RETURNS: the vector register address of the corresponding vector type
*/
...
/******************************************************************************
*
* epicSrcAddrCheck - check if the VPR address passed in is external,
*                    internal, timer, message, or IPI.
*
* NOMANUAL
*
...
STATUS epicIntSourceSet(
	...
	case EPIC_EX_INTERRUPT:
		...
		srcVal &= ~(EPIC_EIVPR_INTR_POLARITY |
					...
					EPIC_EIVPR_PRIORITY_MSK | EPIC_EIVPR_VECTOR_MSK);
		/* set new values */
		srcVal |= (EPIC_EIVPR_POLARITY(polarity) |
				   EPIC_EIVPR_SENS(sense) |
				   EPIC_EIVPR_PRIORITY(priority) | EPIC_EIVPR_VECTOR(vector));
		break;
	...
	case EPIC_IN_INTERRUPT:
		...
		srcVal &= ~(EPIC_IIVPR_INTR_POLARITY |
					EPIC_IIVPR_PRIORITY_MSK | EPIC_IIVPR_VECTOR_MSK);
		/* set new values */
		srcVal |= (EPIC_IIVPR_POLARITY(polarity) |
				   EPIC_IIVPR_PRIORITY(priority) | EPIC_IIVPR_VECTOR(vector));
		break;
	...
	case EPIC_GT_INTERRUPT:
		...
		/* mask off current settings */
		srcVal &= ~(EPIC_GTVPR_PRIORITY_MSK | EPIC_GTVPR_VECTOR_MSK);
		/* set new values */
		srcVal |= (EPIC_GTVPR_PRIORITY(priority) | EPIC_GTVPR_VECTOR(vector));
		break;
	...
	case EPIC_MSG_INTERRUPT:
		...
		/* mask off current settings */
		srcVal &= ~(EPIC_MIVPR_PRIORITY_MSK | EPIC_MIVPR_VECTOR_MSK);
		/* set new values */
		srcVal |= (EPIC_MIVPR_PRIORITY(priority) | EPIC_MIVPR_VECTOR(vector));
		break;
	...
	case EPIC_IPI_INTERRUPT:
		...
		/* mask off current settings */
		srcVal &= ~(EPIC_IPIVPR_PRIORITY_MSK | EPIC_IPIVPR_VECTOR_MSK);
		/* set new values */
...
/******************************************************************************
*
* sysIntShow  - show the vector table
*
* This routine show the vector table.
*
* RETURNS: N/A
*/
void sysIntShow(
	int verbose    /* 1 - display uninitialized vectors too */
)
{
	int i;

	printf(" #\thandler\t\targ\t\tIRQ count\n");
	printf(" ------------------------------------------------\n");
	for( i=0; i<INTERRUPT_TABLESIZE; i++ ){
		INT_HANDLER_DESC *ihd = sysIntTbl[i];

		if( ihd ){

			printf(" 0x%02x\t%08p\t%08x\t%ld\n",
				   i, ihd->vec, ihd->arg, irqCount[i]);
