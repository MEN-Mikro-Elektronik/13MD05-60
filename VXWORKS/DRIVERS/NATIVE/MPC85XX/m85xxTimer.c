...
#if 0
/*
 * ts@men: sysAuxClk support now implemented in sysMPC85xxGlobalTimer.c
 *         using PIC Global Timer 0! This Implementation with
 *		   Fixed Interval Timers doesnt support all ticks per second values
 *		   seamlessly.
 */
...
#ifdef M85XXX_TIMER_INCLUDE_WDT
...
#endif /* M85XXX_TIMER_INCLUDE_WDT */
...
STATUS sysAuxClkRateSet(
	...
	for (ix = 0; ix < (int) NELEMENTS(fitTable); ix++) {
		if (fitPeriod <= fitTable[ix].fitPeriod) {
			if (ix != 0)
				if (fitPeriod <
					((fitTable[ix].fitPeriod +
					  fitTable[ix - 1].fitPeriod) / 2))
					jx = ix - 1;
				else
					jx = ix;
			else
...
#endif /* end of #if 0 due to reimplemented sysAuxClk functions*/
...
* sysWdtConnect
*
* connect a routine to the watchdog interrupt
* This routine specifies the interrupt service routine to be called at
...
STATUS sysWdtRateSet(
	...
	for (ix = 0; ix < (int) NELEMENTS(wdtTable); ix++) {
		if (wdtPeriod <= wdtTable[ix].wdtPeriod) {
			if (ix != 0)
				if (wdtPeriod <
					((wdtTable[ix].wdtPeriod +
					  wdtTable[ix - 1].wdtPeriod) / 2))
					jx = ix - 1;
				else
					jx = ix;
			else
...
/************* sysTimestampConnect ********************************************
*
* connect a user routine to a timestamp timer interrupt
*
...
/******************* sysTimestampEnable ***************************************
*
* initialize and enable a timestamp timer
...
* sysTimestampFreq
*
* get a timestamp timer clock frequency
...
/********************* sysTimestamp *******************************************
*
* get a timestamp timer tick count
...
/********************* sysTimestampLock ***************************************
*
* lock an interrupt and get a timestamp timer tick count
