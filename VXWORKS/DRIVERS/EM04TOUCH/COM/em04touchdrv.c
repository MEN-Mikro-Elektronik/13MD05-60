...
static int TouchRawXToPixel(
	const EM04_TOUCH_CAL *cal,
	u_int16 xres,
	u_int16 raw );
static int TouchRawYToPixel(
	const EM04_TOUCH_CAL *cal,
	u_int16 yres,
	u_int16 raw );
...
/**********************************************************************/
/** Convert raw X value to pixel coordinates
 */
static int TouchRawXToPixel(
	const EM04_TOUCH_CAL *cal,
	u_int16 xres,
...
/**********************************************************************/
/** Convert raw Y value to pixel coordinates
 */
static int TouchRawYToPixel(
	const EM04_TOUCH_CAL *cal,
	u_int16 yres,
	...
	u_int16 x2 )
{
	...
	pixX = TouchRawXToPixel( &th->calibdat, th->init.ds_xres,
							 th->currentRaw.x);
	pixX2 = TouchRawXToPixel( &th->calibdat, th->init.ds_xres, x2 );

	/*--- test if X and X2 are at almost same pos ---*/
	if( ABS( pixX - pixX2 ) > EM04_TOUCH_DEF_THOLD ){
		DBGWRT_2((DBH," X/X2 too far away %d %d\n", pixX, pixX2 ));
		retVal = FALSE;
		goto EXIT;
	}

	pixY = TouchRawYToPixel( &th->calibdat, th->init.ds_yres,
							 th->currentRaw.y);

	/*--- test if X, X2 and Y are inside valid screen area ---*/
	if( (pixX  < 0 - EM04_TOUCH_DEF_THOLD) ||
		(pixX  > (int)th->init.ds_xres + EM04_TOUCH_DEF_THOLD) ||
		(pixX2 < 0 - EM04_TOUCH_DEF_THOLD) ||
		(pixX2 > (int)th->init.ds_xres + EM04_TOUCH_DEF_THOLD) ||
		(pixY  < 0 - EM04_TOUCH_DEF_THOLD) ||
		(pixY  > (int)th->init.ds_yres + EM04_TOUCH_DEF_THOLD) ){
	...
	if( pixX >= th->init.ds_xres ) {
		pixX = th->init.ds_xres - 1;
	}

	if( pixX2 < 0 ) {
		pixX2 = 0;
	}
	if( pixX2 >= th->init.ds_xres ) {
		pixX2 = th->init.ds_xres - 1;
	}

	if( pixY < 0 ) {
		pixY = 0;
	}
	if( pixY >= th->init.ds_yres ) {
		pixY = th->init.ds_yres - 1;
	}

	/* get average value from X/X2 */
	pos->x = (u_int16)((pixX + pixX2) / 2);
